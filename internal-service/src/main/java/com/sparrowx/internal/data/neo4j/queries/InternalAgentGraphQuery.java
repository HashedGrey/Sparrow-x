...
@Component
public class InternalAgentGraphQuery {
    ...
    public InternalAgentGraphQuery(Driver driver) {
        this.driver = driver;
    }
    ...
    public InternalGraphContext read(
        ...
        int safeDepth = clamp(depth <= 0 ? DEFAULT_DEPTH : depth, MAX_DEPTH);
        int safeLimit = clamp(limit <= 0 ? DEFAULT_LIMIT : limit, MAX_LIMIT);

        String graphId = "company-" + UUID.randomUUID();
